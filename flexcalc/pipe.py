...
class Pipe:
    ...
    def add_data(self, local_path):
        ...
        if not '*' in local_path:
            ...
            # Expand and loop:
            folders = sorted(glob.glob(local_path))
    ...
    def _buffer_to_que_(self):
       ...
       for ii, buffer in enumerate(self._buffer_['tot_data']):
            ...
            new_block.meta = io.init_meta()
            
            if 'tot_geom' in self._buffer_.keys():
        ...
        for data in self._data_que_:
            ...
            if self._arg_(argument, 1):
                ...
                if os.path.exists(os.path.join(path, 'meta.toml')):
                    ...
                    #else:
                        meta = io.read_meta(path, 'flexray', sample = samp)
    ...
    def _process_flex_(self, data, count, argument):
        ...
        proj_number = self._arg_(argument, 3)
        ...
        # Read and process:
        proj, meta = process.process_flex(path, samp, skip, memmap_file, proj_number = proj_number) 
    ...
    def process_flex(self, sampling = 1, skip = 1, memmap = False, proj_number = None):
        """
        Read and process FlexRay data.
        """
        self._add_action_('process_flex', self._process_flex_, _ACTION_BATCH_, sampling, skip, memmap, proj_number)
    ...
    def _bh_correction_(self, data, count, argument):
        ...
        data.data = process.equivalent_density(data.data, data.meta, spec['energy'], spec['spectrum'], compound = compound, density = density)
    ...
    def _make_stl_(self, data, count, argument):
        ...
        # Generate STL:
        stl_mesh = process.generate_stl(data.data, data.geometry)
    ...
    def _merge_detectors_(self, data, count, argument):
        ...
        process.append_tile(data.data, data.meta['geometry'], total, tot_geom)
    ...
    def merge_detectors(self, memmap = False, preview = False):
        """
        ...
        """
        self._add_action_('merge_detectors', self._merge_detectors_, _ACTION_STANDBY_, memmap, preview)
    ...
    def _merge_volume_(self, data, count, argument):
        ...
        if count == 1:    
            ...
            offset = numpy.int32(io.mm2pixel(max(vol_z) - min(vol_z), data.geometry))
            ...
            b0 = array.volume_bounds(data.data.shape, self._data_que_[0].meta['geometry'])
            
            overlap = 0            
            for ii in range(1, len(self._data_que_)):
                
                b1 = array.volume_bounds(data.data.shape, self._data_que_[ii].meta['geometry'])
        ...
        offset = numpy.int32(io.mm2pixel(vol_z - vol_z0, data.geometry))
    ...
    def _fdk_(self, data, count, argument):        
        ...
        vol_shape = self._arg_(argument,2)
        
        if vol_shape:
            vol = numpy.zeros(vol_shape, dtype = 'float32')
        else:
            vol = project.init_volume(data.data, data.meta['geometry'])
    ...
    def FDK(self, em = 0, sirt = 0, vol_shape = None):
        """
        Reconstruct using FDK. Use em, sirt to specify a number iterations to apply with SIRT or EM after FDK is computed.
        """
        self._add_action_('FDK', self._fdk_, _ACTION_BATCH_, em, sirt, vol_shape)        
            
    def _sirt_m_(self, data, count, argument):        
        """
        Unlike FDK, SIRT may process several projection datasets. For that we will need to create a buffer.
        """        
        ...
        # If this is the last call:   
           ...
           data.data = self._buffer_['tot_data']
           data.type = 'volume'
           data.meta = meta
    ...
    def SIRT_M(self, iterations = 10, block_number = 40, mode = 'random', vol_shape = None, memmap = False):
        ...
        """
        self._add_action_('SIRT_M', self._sirt_m_, _ACTION_STANDBY_, iterations, block_number, mode, vol_shape)
    ...
    def _ramp_(self, data, count, argument):
        ...
        data.data = array.pad(data.data, dim, width, mode)
    ...
    def _shape_(self, data, count, argument):
        ...
        if (myshape != shape):
            ...
            for dim in range(3):
                ...
                if crop < 0:
                    # Crop case:
                    data.data = array.crop(data.data, dim, -crop, symmetric = True)
            
                elif crop > 0:
                    # Pad case:
                    data.data = array.pad(data.data, dim, crop, symmetric = True)
    ...
    def _bin_(self, data, count, argument):
        ...
        mode = self._arg_(argument, 0)
        
        if mode == 'volume':
            data.data = array.bin(data.data)
        else:
            data.data = array.bin(data.data, dim = 0)
            data.data = array.bin(data.data, dim = 2)
    ...
    def bin(self, mode = 'volume'):
        """
        Bin the data for the volume data (in all dimensions) or of the projection data (in dim = [0, 2])
        Use with care! Bin is not take into account by read_all_meta for instance!!!
        """
    ...
    def _crop_(self, data, count, argument):
        ...
        data.data = array.crop(data.data, dim, width)
    ...
    def _auto_crop_(self, data, count, argument):
        ...
        a,b,c = process.bounding_box(data.data)
        ...
        data.data = array.crop(data.data, 0, [a[0], sz[0] - a[1]])
        data.data = array.crop(data.data, 1, [b[0], sz[1] - b[1]])
        data.data = array.crop(data.data, 2, [c[0], sz[2] - c[1]])
    ...
    def _marker_normalization_(self, data, count, argument):
        ...
        # Find the marker:
        a,b,c = process.find_marker(data.data, data.meta)    
    ...
    def _cast2type_(self, data, count, argument):
        ...
        data.data = array.cast2type(data.data, dtype, bounds)
    ...
    def _shift_(self, data, count, argument):
        ...
        data.data = process.translate(data.data, shift = shift, axis = dim)
    ...
    def _register_volumes_(self, data, count, argument):
        ...
        if count == 1:
            ...
            # Register volumes
            T, R = process.register_volumes(self._buffer_['fixed'], data.data, subsamp = 2, use_CG = True, monochrome = False)
            
            # Resample the moving volume:
            data.data = process.affine(data.data, R, T)
    ...
    def _soft_threshold_(self, data, count, argument):
        ...
        process.soft_threshold(data.data, self._arg_(argument, 0), self._arg_(argument,1))
    ...
    def _histogram_(self, data, count, argument):
        ...
        process.histogram(data.data[::2,::2,::2], log = log)
    ...
    def _equalize_resolution_(self, data, count, argument):
        ...
        if fact != 1:
            ...
            data.data = process.scale(data.data, fact)
        ...
        if any(myshape != shp_max):
            ...
            for dim in range(3):
                ...
                if crop < 0:
                    # Crop case:
                    data.data = array.crop(data.data, dim, crop)
            
                elif crop > 0:
                    # Pad case:
                    data.data = array.pad(data.data, dim, crop)
